...
public class KaChingDbAdapter {
	...
	public long createRecord(Date date, String type, double value, String description, String user) {
	...
	public boolean updateRecord(long rowId, Date date, String type, Double value, String description, String user){
		...
		If I understand your question, Where clause is optional, it may be null, if you don't provide where clause it will update all the rows in a table.
		It is just a sql thing, like if you don't supply where condition it will update all rows. documentation clearly says that,
		Parameters whereClause the optional WHERE clause to apply when updating. Passing null will update all rows.
		SqlliteData Update*/
		ContentValues values = new ContentValues();
		values.put(RecordEntry.COLUMN_NAME_DATETIME, date.getTime() );
		values.put(RecordEntry.COLUMN_NAME_TYPE, type);
		...
		values.put(RecordEntry.COLUMN_NAME_DESCRIPTION, description);
		values.put(RecordEntry.COLUMN_NAME_USER, user);
		return database.update(RecordEntry.TABLE_NAME, values, RecordEntry.COLUMN_NAME_ID + "=" + rowId, null) > 0;

	}
	
	 public boolean deleteTodo(long rowId) {
		 return database.delete(RecordEntry.TABLE_NAME, RecordEntry.COLUMN_NAME_ID + "=" + rowId, null) > 0;
	 }
